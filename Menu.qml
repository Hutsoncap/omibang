...
Item {
  ...
  property string searchConfigPath: Quickshell.env("HOME") + "/.config/omarchy/extensions/omarchy-search.json"
  property string defaultBang: "ddg"
  ...
  readonly property string bangScriptPath: decodeURIComponent(String(Qt.resolvedUrl("scripts/bangs.py")).replace(/^file:\/\//, ""))
  readonly property string defaultBangScriptPath: decodeURIComponent(String(Qt.resolvedUrl("scripts/default_bang.py")).replace(/^file:\/\//, ""))
  property string pendingBangTrigger: ""
  ...
  function loadSearchSettings(raw) {
    root.defaultBang = "ddg"
    try {
  ...
  function bangInput(query) {
    ...
    if (defaultMatch) {
      return {
        query: normalized,
        trigger: String(defaultMatch[1]).toLowerCase(),
        terms: "",
    ...
    var selected = entered.length > tokenLength && /\s/.test(entered.charAt(tokenLength))
    return {
      query: normalized,
      trigger: String(match[1]).toLowerCase(),
      terms: String(match[2] || "").trim(),
      defaultRequest: false,
      selected: selected,
      setDefault: false
    }
  ...
  function expandBangTemplate(template, terms) {
    var url = String(template || "")
    var encoded = encodeURIComponent(String(terms || ""))
    var placeholders = ["{searchTerms}", "{{{s}}}", "{{s}}", "%s"]
    for (var i = 0; i < placeholders.length; i++) {
      if (url.indexOf(placeholders[i]) < 0) continue
      return url.split(placeholders[i]).join(encoded)
    }
    return url
  }
  ...
  function primeDefaultBang() {
    var trigger = String(root.defaultBang || "").toLowerCase()
    if (!trigger || root.bangCache[trigger] || root.resolvingBangTrigger === trigger) return
    root.pendingBangTrigger = trigger
    if (!bangResolveProc.running) bangResolveTimer.restart()
  }

  function scheduleBangSearch(query) {
    root.invalidBangTrigger = ""
  ...
  function resolvePendingBang() {
    if (bangResolveProc.running || !root.pendingBangTrigger) return
    var trigger = root.pendingBangTrigger
    root.pendingBangTrigger = ""
  ...
  function bangLoadingRow(query) {
    var input = root.bangInput(query)
    if (!input) return null
    return {
      itemId: "web-search.bang-loading",
      kind: "status",
      icon: "",
      iconFont: "",
      appIcon: "",
      appId: "",
      label: input.setDefault ? "Checking !" + input.trigger + "…" : (input.defaultRequest ? "Preparing default search…" : "Finding !" + input.trigger + "…"),
      target: "",
      detail: "",
      path: "Web Search",
      childCount: 0,
  ...
  function searchHeaderHint() {
    if (root.dmenuActive || !root.filterText || root.filterText.length > 12) return ""
    var query = root.filterText.trim()
    ...
    if (result && result.query === query) {
      if (result.setDefault)
        return result.trigger === root.defaultBang ? "Current default" : "Enter saves default"
      if (result.terms) return "Enter searches"
      if (result.selected) return "Type query · Enter"
      return "Tab selects"
    }
    if (root.bangSearchPending) return "Checking…"
    if (root.invalidBangTrigger) return "Not in Helium"
    return ""
  }

  function defaultBangPromptRow() {
    return {
      itemId: "web-search.default-prompt",
      kind: "status",
      icon: "",
      iconFont: "",
      appIcon: "",
      appId: "",
      label: "Set default iBang",
      target: "",
      detail: "",
  ...
  function bangCatalogRow() {
    return {
      itemId: "web-search.bang-catalog",
      kind: "action",
      icon: "",
      iconFont: "",
      appIcon: "",
  ...
  function invalidBangRow(trigger) {
    return {
      itemId: "web-search.invalid-bang",
      kind: "status",
      icon: "󰅖",
      iconFont: "",
      appIcon: "",
      appId: "",
      label: "!" + String(trigger) + " not found",
      target: "",
      detail: "",
      path: "Search settings",
      childCount: 0,
  ...
  function rebuildDisplay() {
    ...
    if (query) {
      ...
      else if (root.bangSearchPending) {
        var loadingRow = root.bangLoadingRow(query)
        if (loadingRow) rows.unshift(loadingRow)
      }
      else if (query === "!!") {
        rows.unshift(root.defaultBangPromptRow())
      }
      else if (query === "!?") {
        rows.unshift(root.bangCatalogRow())
      }
      else if (root.invalidBangTrigger) {
        rows.unshift(root.invalidBangRow(root.invalidBangTrigger))
      }
    } else {
  ...
  function setFilter(nextFilter) {
    ...
    root.scheduleBangSearch(nextFilter)
  ...
  Timer {
    ...
    interval: 80
    repeat: false
    onTriggered: root.resolvePendingBang()
  }
  ...
  Process {
    ...
    stdout: SplitParser {
      onRead: function(data) { bangResolveProc.collected += data }
    }
    onExited: {
  ...
  FileView {
    ...
    printErrors: false
    onLoaded: {
      root.loadSearchSettings(text())
      root.primeDefaultBang()
    }
    onLoadFailed: root.primeDefaultBang()
    onFileChanged: reload()
  }
      ...
      Item {
        ...
        Keys.onPressed: function(event) {
          ...
          if (event.key === Qt.Key_Delete) {
            ...
            event.accepted = true
          } else if (event.key === Qt.Key_Tab && root.requestBangInMenu()) {
            event.accepted = true
